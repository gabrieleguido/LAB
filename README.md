...
```bash
docker load -i lab-grader-progetto-finale_1.0.5.tar.gz
```
## Prerequisiti
...
```bash
docker run --network host lab-grader-progetto-finale:1.0.5 2144286
```
...
docker run --network host \
  -v "$(pwd)/output:/output" \
  lab-grader-progetto-finale:1.0.5 2144286 --machine -o /output/report.json
```
